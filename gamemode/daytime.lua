...
// table.
daylight = { };
print("Loading Day/Night")

// variables.
local DAY_LENGTH	= (24 * 120) // 24 hours
local DAY_START		= (5 * 120); // 5:00am
local DAY_END		= (18.5 * 120); // 6:30pm
local DAWN			= ( DAY_LENGTH / 4 );
local DAWN_START	= DAWN - (DAWN / 3);
local DAWN_END		= DAWN + (DAWN / 3);
local NOON			= DAY_LENGTH / 2;
local DUSK			= DAWN * 3;
local DUSK_START	= DUSK - (DAWN / 3);
local DUSK_END		= DUSK + (DAWN / 3);
...
print("Day/Night RealTime: " .. tostring(GetRealTime()))
...
function daylight:init( )
	print("Day/Night Init")
	// clear think time.
	self.nextthink = 0;
	// midnight?
	self.minute = GetRealTime();
...
function daylight:think( )
	...
	// progress the time.
	self.minute = GetRealTime() --self.minute + 1;
	for _, ply in pairs(player.GetHumans()) do
		umsg.Start("GetRealTime", ply)
			umsg.Long(self.minute)
		umsg.End()
	end
